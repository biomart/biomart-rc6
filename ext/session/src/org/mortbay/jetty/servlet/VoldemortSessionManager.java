...
public class VoldemortSessionManager extends AbstractSessionManager {
    protected static final String __sessionPrefix = "session:";
    protected static final String __attributesSuffix = ":attributes";

    protected final TypeReference<HashMap<String,Object>[]> _typeRef = new TypeReference<HashMap<String,Object>[]>() {};
    ...
    public static class SessionData {
        ...
        @JsonIgnore private Map<String,Object> _attributes;
        ...
        public String toString () {
            return "Session id="+_id+",lastNode="+_lastNode+
...
//                        Log.debug("Session not stale "+session._data);
    ...
    private String getKey(String id) {
        return __sessionPrefix + id;
    }

    private String getAttributesKey(String key) {
        return key + __attributesSuffix;
    }

    private SessionData loadData(String key) throws Exception {
        VoldemortDataStore<String,Object> store = getConnection();
        String attrKey = getAttributesKey(key);
        SessionData data = null;

        synchronized (store) {
            String str = (String)store.get(key);
            Map<String,Object> attr = (Map<String,Object>)store.get(attrKey);

            if (str != null) {
                data = (SessionData)_mapper.readValue(str, SessionData.class);
                if (attr != null) {
                    data._attributes = attr;
                } else {
                    data._attributes = new HashMap<String,Object>();
                }
            }
        }
    ...
    private void storeData(SessionData data) throws Exception {
        VoldemortDataStore<String,Object> store = getConnection();
        String key = getKey(data.getId());
        String attrKey = getAttributesKey(key);

        synchronized (store) {
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            _mapper.writeValue(baos, data);
            store.put(key, baos.toString());

            store.put(attrKey, data._attributes);
        }
    }
    ...
    throws Exception {
        ...
        String key = getKey(data.getId());
        store.delete(key);
        store.delete(getAttributesKey(key));
        if (Log.isDebugEnabled())
     ...
     * @throws SQLException
     */
    private VoldemortDataStore<String,Object> getConnection () {
