...
public class Container extends MartConfiguratorObject {
	...
	public void addFilter(Filter filter) {
		//check if the filter is already exist
	...
	public void addAttribute(int index, Attribute attribute) {
		...
		if(this.getParentConfig()!=null) {
			...
			if(this.getParentConfig().getAttributeByName(attribute.getName(), new ArrayList<String>())!=null) {
				//rename attribute
...
//		return element!=null ? (Filter)element : null;
	...
	public  void synchronizedFromXML() {
		for(Container subContainer: this.containerList) {
			subContainer.synchronizedFromXML();
		}
		for(Attribute subAtt: this.attributeList) {
			subAtt.synchronizedFromXML();
		}
		for(Filter subFil: this.filterList) {
			subFil.synchronizedFromXML();
		}
