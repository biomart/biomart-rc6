...
public final class Config extends MartConfiguratorObject {
	...
	public Config(Element element) {
		...
		for(Element impElement: impElementList) {
			ElementList importable = new ElementList(impElement);
		...
		for(Element expElement: expElementList) {
			ElementList exportable = new ElementList(expElement);
	...
	public  void synchronizedFromXML() {
		//rootcontainer
		this.getRootContainer().synchronizedFromXML();		
		this.setObjectStatus(ValidationStatus.VALID);
	}
	
	/*
	 * called after all filters/attributes are synchronized
	 * which includes importable, exportable, attributelist, filterlist
	 */
	@Deprecated
	public ValidationStatus synchronizedList() {
		//importable
		for(ElementList importable: this.importableList) {
			importable.synchronizedFromXML();
		}
		//exportable
		for(ElementList exportable: this.exportableList) {
			exportable.synchronizedFromXML();
		}
		//get all attributes
		for(Attribute att: this.attributeMap.values()) {
			if(att.isAttributeList()) {
				String[] atts = att.getPropertyValue(XMLElements.ATTRIBUTELIST).split(",");
				for(String attStr: atts) {
					Attribute attItem = this.getAttributeByName(attStr,null);
					if(attItem!=null)
						att.addAttribute(attItem);
				}
			}
		}
		//get all filters
		for(Filter filter: this.filterMap.values()) {
			if(filter.isFilterList()) {
				String[] filters = filter.getPropertyValue(XMLElements.FILTERLIST).split(",");
				for(String _filStr: filters) {
					Filter filterItem = this.getFilterByName(_filStr, null);
					if(filterItem!=null)
