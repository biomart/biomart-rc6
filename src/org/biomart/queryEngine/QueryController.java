...
public final class QueryController {
    ...
    public void runQuery(OutputStream outputHandle) throws TechnicalException, IOException {
		try {
            ...
            processorObj.beforeQuery(queryRunnerObj.query, outputHandle);

			queryRunnerObj
                    // Get new OutputStream (may have changed)
                    .setOutputHandle(processorObj.getOutputStream())
                    .runQuery();

            // Perform post processing (if any)
            processorObj.afterQuery();
    ...
    private ProcessorInterface initializeProcessor() {
        try {
            ...
            for (String field : p.getFieldNames()) {
                ...
                if (p.isUserDefined(field)) {
                    value = StringEscapeUtils.unescapeJava(queryValidator.getProcessorParams().get(field));
                } else {
                    value = StringEscapeUtils.unescapeJava(queryValidator.getProcessorConfig().get(field));
                }
