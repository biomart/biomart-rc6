...
public class UpdateMartModel {
	...
	private Map<String,List<String>> getDbInfoFromUrl(Dataset ds) {
		...
		List<DatasetFromUrl> tmpDsList = McGuiUtils.INSTANCE.getDatasetsFromURL(fullhost+":"+port+path, modifiedName);
	...
	public void updateDatasetTables(Mart mart, Dataset ds, Map<String,List<String>> newTables, 
			...
			Set<DatasetColumn> newInDsColumn, Set<DatasetTable> dropInMartTable, 
			Set<DatasetTable> dropInDsTable, Set<DatasetColumn> dropInMartColumn, Set<DatasetColumn> dropInDsColumn, boolean isNewForDataset) {
		...
		for(DatasetTable droppedDst: droppedList) {
			...
			for(Column droppedDsc: droppedDst.getColumnList(datasetName)) 
				this.dropColumnFromDataset(mart, (DatasetColumn)droppedDsc, datasetName, dropInMartColumn, dropInDsColumn);	    		
		}
	...
	private void updateDsFromUrl8(Dataset ds) {
		...
		List<MartPointer> mpList = mart.getMartRegistry().getPortal().getRootGuiContainer().getMartPointerListforMart(mart);
		if(mpList.isEmpty())
			return;
		PartitionTable mainPT = mart.getSchemaPartitionTable();
	...
	private void updateDatasetForDB(Dataset ds) throws MartBuilderException {
		...
		this.updateDatasetTables(mart, ds, 
				tables, newInMartTable, newInDsTable, newInMartColumn, newInDsColumn,
			...
			for(DatasetTable newDst: newInMartTable) {
				for(Column column: newDst.getColumnList()) {
					...
					if(column.getName().endsWith(Resources.get("keySuffix"))) {
						...
						//update relation for the new table, link it with the highest level of main table
	...
	private void updateDsWith2SchemaPartitions(Dataset ds) throws MartBuilderException {
		...
		McSQL mcsql = new McSQL();
		//get all database info
		Map<String,List<String>> tables = mcsql.getMartTablesInfo(ds.getDataLinkInfoNonFlip().getJdbcLinkObject(), dsName);
		if(tables.isEmpty()) {//skip the url for now {
			throw new MartBuilderException("database naming convention error");
		}
	...
	private void updateConfigForNewColumn(Config config, DatasetColumn newDsc, Container newAC, Container newFC) {
		...
		Container foundAttContainer = this.findContainer(config, newAC, newFC,tableName+Resources.get("ATTRIBUTESUFFIX"));
		Attribute newA;
		if(foundAttContainer!=null)
			newA = this.createNaiveAttribute(config,foundAttContainer, (DatasetColumn)newDsc);
		else {
		...
		Container foundFilterContainer = this.findContainer(config, newAC, newFC,tableName+Resources.get("FILTERSUFFIX"));
	...
	private Attribute createNaiveAttribute(Config mart, Container container, DatasetColumn dsc) {
		...
		container.setVisibleModified(true); //TODO: if a container has both ADD and REMOVE?
		return newA;
	}
