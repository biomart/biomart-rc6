...
class MySQLDialect extends DatabaseDialect {
	...
	protected String quote(String value) {	// overriden by mysql and mssql
		return MYSQL_QUOTE + value + MYSQL_QUOTE;
	}
	...
	protected String getSourceTableFullyQualifiedName(final MartConstructorAction action, String tableName) {
		return action.getMartBuilderCatalog().getSourceDatabaseName() + "." + tableName;	// because in mysql schema<=>database
	}
	@Override
	protected String getTargetTableFullyQualifiedName(final MartConstructorAction action, String tableName) {
		return action.getMartBuilderCatalog().getTargetDatabaseName() + "." + tableName;	// because in mysql schema<=>database
	}
	@Override
	protected final String addRenameStatement(final String schemaName, final String oldTableName, final String newTableName) {
		return RENAME + " " + TABLE + " " + schemaName + "." + oldTableName + " " + TO + " " + schemaName + "." + newTableName;
	}
	@Override
	...
	@Override
	protected String addIndexPrefix(final String schemaName) {
		return "";
	}
