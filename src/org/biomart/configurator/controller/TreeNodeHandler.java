...
public class TreeNodeHandler {
    ...
    public boolean requestCreateElementList(McTreeNode node, PortableType type) {
		LinkItemsDialog linkDialog = new LinkItemsDialog((Config)node.getObject(),type,"");
		linkDialog.setVisible(true);
		ElementList el = linkDialog.getElementList();
		if(el == null)
			return false;
		//add to config
		((Config)node.getObject()).addElementList(el);
		//add to gui
		McTreeNode elNode = new McTreeNode(el);
		node.add(elNode);
    	return true;
    }
    
    public ValidationStatus requestValidateRegistry() {
    	return Validation.validateObject(McGuiUtils.INSTANCE.getRegistryObject());
    }
